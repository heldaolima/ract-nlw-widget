import { Camera, CurrencyBtc, Trash } from "phosphor-react";
...
export function ScreenshotButton({ onScreenshotTook, screenshot }: ScreenshotButtonProps) {
    const [isTakingScreenshot, setIsTakingScreenshost] = useState(false);

    async function handleTakeScreenshot() {
        setIsTakingScreenshost(true);

        const canvas = await html2canvas(document.querySelector('html')!);
        const base64image = canvas.toDataURL('image/png');
        
        onScreenshotTook(base64image);   
        setIsTakingScreenshost(false);
    }
